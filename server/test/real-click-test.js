...
// retries through it.
// Run: node server/test/real-click-test.js
...
console.log("boot: lab server spawning on ws://127.0.0.1:" + PORT, "pid:", server.pid);
...
try {
  ...
  const status = await callRetry("evaluate", {
    expression:
      "document.getElementById('lab-frame').contentDocument.getElementById('inner-status').textContent",
  });
