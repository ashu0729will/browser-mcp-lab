// Visual demo: start a SLOW (5s) virtual-cursor animation on the examples page
// via our extension, then capture a mid-flight screenshot so the moving arrow
// and its trajectory are visible in the saved PNG.
// Run: node server/test/demo-cursor.js
...
try {
  ...
  const demo = await call("evaluate", {
    expression: `(() => {
      let c = document.getElementById('__bml_cursor');
      if (!c) {
