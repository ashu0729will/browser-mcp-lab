...
将当前浏览器会话连接到本地 MCP 客户端的浏览器自动化扩展与服务器。Node.js 22+，无运行时 npm 依赖。0.4.1 修复超限消息中断连接的问题；0.4.0 的重点是连接可诊断、浏览器目标明确、断开后可恢复。
...
客户端对单条 MCP 消息有自己的上限；一旦服务器发出的帧超过它，客户端会报
「mcp server sent an oversized message」并**丢弃服务器**，之后所有工具都会变成
「unknown mcp tool」。因此服务器对每条输出消息设上限（`BSM_MAX_MESSAGE_BYTES`，默认 1 MiB，最小
64 KiB）：超限的**工具结果**改为返回有界的 `RESULT_TOO_LARGE` 工具错误，并说明连接仍然正常、
其他工具仍可用；服务器不会退出，也无需重启。这是服务器侧保证——上限仍需低于客户端自身的消息上限。

遇到该错误时不要重复同样的调用，改为取更小的结果：用 `read` 读具体元素、用较小的
`snapshot.max`、或只取需要的字段，而不是把整份接口/JSON 响应读出来；确实需要更大结果时，
为该服务器提高 `BSM_MAX_MESSAGE_BYTES`。
...
- `BSM_MAX_MESSAGE_BYTES`：单条输出消息的上限，默认 1048576（1 MiB），最小 65536（64 KiB）。
  超过上限的工具结果不会发出，而是返回 `RESULT_TOO_LARGE` 错误（见上）。请保持在**客户端自身
  上限之下**；不确定时不要提高，否则会重新触发客户端丢弃服务器的故障。
...
- 旧扩展不支持持久客户端身份（0.4.0 起）时应升级、重载，不要将旧版与新版混用作多浏览器自动化。
- 单次读取的页面/接口数据可能很大；超限结果会被替换为 `RESULT_TOO_LARGE` 错误，而不是中断连接。
