...
```text
在本机安装并启用 browser-mcp-lab。

1. 克隆仓库（已存在则跳过）：
   git clone https://github.com/ashu0729will/browser-mcp-lab.git

2. 加载扩展（需手动完成）：
   - Firefox：about:debugging → 临时加载附加组件 → <仓库>/extension/manifest.json
   - Chrome / Edge：chrome://extensions → 加载已解压的扩展程序 → <仓库>/extension/

3.（可选）注册原生消息宿主：
   node <仓库>/native-host/install.js
   Chromium 系需把扩展 ID 写入 <仓库>/native-host/chrome-extension-id.txt 后重新执行。
...
4. 在 ~/.zcode/cli/config.json 的 mcp.servers 下新增（仅新增，不覆盖其他配置）：
   ...
   服务器零依赖，无需 npm install。

5. 安装技能：把 <仓库>/plugin/skills/browser-mcp-lab 复制到 ~/.zcode/skills/
